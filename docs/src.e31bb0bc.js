...
},{"topojson-client":"../node_modules/topojson/node_modules/topojson-client/index.js","topojson-server":"../node_modules/topojson/node_modules/topojson-server/index.js","topojson-simplify":"../node_modules/topojson/node_modules/topojson-simplify/index.js"}],"winners_years_switch.csv":[function(require,module,exports) {
module.exports = "/winners_years_switch.d77abdf4.csv";
},{}],"index.js":[function(require,module,exports) {
// module global variables
var d3 = require('d3');

var topo = require('topojson'); //projection
...
var svg = d3.select("#map").append("svg").attr("width", width).attr("height", height - 150);
...
function extractData() {
  d3.csv(csvFile).then(function (data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      winner.push({
        id: temp_id,
        winner_name: [data[i]["party2000"] + "2000", data[i]["party2004"] + "2004" + data[i]["Switch_2000_2004"], data[i]["party2008"] + "2008" + data[i]["Switch_2004_2008"], data[i]["party2012"] + "2012" + data[i]["Switch_2008_2012"], data[i]["party2016"] + "2016" + data[i]["Switch_2012_2016"]]
      });
    }
...
function drawScreen(winner) {
  ...
  d3.json("https://cdn.jsdelivr.net/npm/us-atlas@3/counties-10m.json").then(function (json) {
    ...
    svg.selectAll('path').data(counties.features).enter().append('path').attr("id", function (d) {
      //console.log(d);
...
function fillMap(year) {
  if (year == 1) {
    d3.selectAll(".republican2000").style("fill", "pink");
    d3.selectAll(".democrat2000").style("fill", "lightblue");
  } else if (year == 2) {
    d3.selectAll(".republican2004False").style("fill", "pink");
    d3.selectAll(".democrat2004False").style("fill", "lightblue");
    d3.selectAll(".republican2004True").style("fill", "red");
    d3.selectAll(".democrat2004True").style("fill", "blue");
  } else if (year == 3) {
    d3.selectAll(".republican2008False").style("fill", "pink");
    d3.selectAll(".democrat2008False").style("fill", "lightblue");
    d3.selectAll(".republican2008True").style("fill", "red");
    d3.selectAll(".democrat2008True").style("fill", "blue");
  } else if (year == 4) {
    d3.selectAll(".republican2012False").style("fill", "pink");
    d3.selectAll(".democrat2012False").style("fill", "lightblue");
    d3.selectAll(".republican2012True").style("fill", "red");
    d3.selectAll(".democrat2012True").style("fill", "blue");
  } else if (year == 5) {
    d3.selectAll(".republican2016False").style("fill", "pink");
    d3.selectAll(".democrat2016False").style("fill", "lightblue");
    d3.selectAll(".republican2016True").style("fill", "red");
    d3.selectAll(".democrat2016True").style("fill", "blue");
  }
}
/**
...
function displayCountyInfo() {
  ...
  var id = "";
  var options = document.querySelectorAll("option"); //console.log(options[0]);
  ...
  id = id.id.substring(1); // i want to dim every thing except the chosen county
  ...
  d3.select("#c" + id + "." + classes[0]).style("fill", palete[0]).transition().delay(750).style("fill", "grey").transition().delay(750).style("fill", palete[1]).transition().delay(750).style("fill", "grey").transition().delay(750).style("fill", palete[2]).transition().delay(750).style("fill", "grey").transition().delay(750).style("fill", palete[3]).transition().delay(750).style("fill", "grey").transition().delay(750).style("fill", palete[4]);
}
},{"d3":"../node_modules/d3/index.js","topojson":"../node_modules/topojson/index.js","./winners_years_switch.csv":"winners_years_switch.csv"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
